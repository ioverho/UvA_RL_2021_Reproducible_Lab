...
from continuous_trpo.model import Actor, Critic
from continuous_trpo.utils.utils import get_action, save_checkpoint, stats_to_writer, format_tf
from continuous_trpo.utils.running_state import ZFilter
from continuous_trpo.hparams import HyperParams as hp
...
parser.add_argument('--logdir', type=str, default='continuous_trpo/logs',
                    help='tensorboardx logs directory')
parser.add_argument('--logdir_json', type=str, default='continuous_trpo/logs_json',
                    help='Json logs directory (for matplotlib plotting)')
args = parser.parse_args()

if args.algorithm == "PG":
    from continuous_trpo.agent.vanila_pg import train_model
elif args.algorithm == "NPG":
    from continuous_trpo.agent.tnpg import train_model
elif args.algorithm == "TRPO":
    from continuous_trpo.agent.trpo_gae import train_model
elif args.algorithm == "PPO":
    from continuous_trpo.agent.ppo_gae import train_model
else:
...
if __name__=="__main__":
    ...
    logdir_json = args.logdir_json
    env = gym.make(args.env)

    # Create an empty output direction
    output_dir = "{}/{}".format(logdir_json, args.env)
    ...
    for session in range(num_sessions + 1):
        ...
        if args.load_model is not None:
            saved_ckpt_path = os.path.join(os.getcwd(), 'continuous_trpo/save_model', str(args.load_model))
        ...
        for iter in range(num_iters):
            ...
            if iter % logging_freq:
                ...
                model_path = os.path.join(os.getcwd(),'continuous_trpo/save_model')
